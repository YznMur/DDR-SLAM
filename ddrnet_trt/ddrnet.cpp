...
cv::Mat ddrnetPredict( cv::Mat &img, char *trtModelStream, size_t size ){
    ...
    static IRuntime* runtime = createInferRuntime(gLogger);
    static ICudaEngine* engine = runtime->deserializeCudaEngine(trtModelStream, size);
    static IExecutionContext* context = engine->createExecutionContext();
...
//        cv::resize(out, real_out, real_out.size());
        ...
        // cv::imwrite("output/" + f, mask);
        cv::Mat result;
        cv::addWeighted(pr_img,1,mask,1,1,result);
        cv::resize(result,result,cv::Size(w_img,h_img));
        cv::imwrite("output/" + f, result);
        delete prob;
        delete data;
    }
    return 0;
